...
Some `ls`es later, I found the place where all my files were placed. I moved them outside of the docker container and erased the container with `docker volume rm nextcloud_app_data` after ensuring all the related containers and images were removed too.
...
The extra folder you see in the `caddy/` directory is required in the caddy's docker-compose (where I host some static stuff from).
